...
void Spell::DoSummonWild(SpellEffectIndex eff_idx, uint32 forceFaction)
{
    ...
    for(int32 count = 0; count < amount; ++count)
    {
        ...
        if(Creature *summon = m_caster->SummonCreature(creature_entry, px, py, pz, m_caster->GetOrientation(), summonType, duration))
        {
            ...
            summon->SetCreatorGUID(m_caster->GetGUID());     //created by
	    summon->SetOwnerGUID(m_caster->GetGUID());	    //summoned by
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 45204: // Clone 
                    unitTarget->CastSpell(m_caster, damage, true);//unitTarget->GetTypeId() = TYPEID_PLAYER !!! m_caster->GetTypeId() = TYPEID_UNIT
                    break;
